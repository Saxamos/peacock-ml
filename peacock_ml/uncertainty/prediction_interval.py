...
def compute_prediction_interval(model, data_eval, percentile=95):
    ...
    A function that compute the prediction interval based on the predictions of each trees of a random forest.
        ...
        Returns
        -------
        lower_bound_prediction_list, upper_bound_prediction_list: Two arrays of prediction intervals
    ...
    lower_bound = (100 - percentile) / 2.0
    upper_bound = 100 - lower_bound

    # this is the matrix of each predictions for each trees
    decision_tree_prediction_matrix = [
        _compute_prediction_of_each_tree(decision_tree_list, data_eval[i])
        for i in range(len(data_eval))
    ]

    lower_bound_prediction_list = [
        np.percentile(prediction_list, lower_bound)
        for prediction_list in decision_tree_prediction_matrix
    ]
    upper_bound_prediction_list = [
        np.percentile(prediction_list, upper_bound)
        for prediction_list in decision_tree_prediction_matrix
    ]
...
def _compute_prediction_of_each_tree(decision_tree_list, row):
    ...
    return [
        decision_tree.predict(reshaped_row)[0] for decision_tree in decision_tree_list
    ]
