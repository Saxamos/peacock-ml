...
def find_weights_and_compute_score(df_train, df_test, epsilon=0.01, random_state=None):
    ...
    xgb_params = {
        ...
        "eta": 1,
        "silent": 1,
        "num_class": len(df_train.y.unique()),
    ...
    predictions = _train_and_predict(
        df_train, df_test, initial_weights, xgb_params, best_iteration
    )
    ...
    while current_iteration <= NUMBER_OF_ITERATION and delta > epsilon:
        predictions = _train_and_predict(
            df_train, df_test, weights, xgb_params, best_iteration
        )
...
def _find_best_iteration(df_train, xgb_params, random_state):
    X_train, X_eval, y_train, y_eval = sk.model_selection.train_test_split(
...
def _train_and_predict(df_train, df_test, weights, xgb_params, best_iteration):
    dtrain = xgb.DMatrix(
        df_train.iloc[:, :-1].values, df_train.y.values, weight=weights
    )
...
def _compute_delta(new_weight_by_class, weight_by_class):
    delta_array = [
        abs(new_weight_by_class[cl] - weight_by_class[cl])
        for cl in weight_by_class.keys()
    ]
