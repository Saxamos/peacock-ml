...
from peacock_ml.data_generator.unbalanced_data_generator import (
    create_weighted_multiclass_dataset,
)
...
class TestCreateWeightedMulticlassDataset(unittest.TestCase):
    def test_create_weighted_multiclass_dataset_should_return_expected_weight_by_class_list(
        self,
    ):
        # Given
        n_samples = 500
        ...
        df_train, df_test = create_weighted_multiclass_dataset(
            n_samples, n_classes, weights, random_state=6
        )

        # When
        weight_by_class_list = find_weights_and_compute_score(
            df_train, df_test, random_state=6
        )

        # Then
        expected_weight_by_class_list = [
            {0: 0.1625, 1: 0.65, 2: 0.1875},
