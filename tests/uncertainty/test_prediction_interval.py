...
from peacock_ml.uncertainty.prediction_interval import compute_prediction_interval

np.random.seed(6)  # set random seed for test


class TestComputePredictionInterval(unittest.TestCase):
    def test_90_percentile_prediction_interval_has_90_percent_of_ground_truth_inside_the_interval(
        self,
    ):
        # Given
        boston = load_boston()
        ...
        # When
        lower_error_list, upper_error_list = compute_prediction_interval(
            rf, data_eval, percentile=90
        )
        ...
        ground_truth_inside_interval_percentage = ground_truth_inside_interval / float(
            len(target_eval)
        )
    ...
    def test_compute_prediction_interval_with_stub_model(self):
        ...
        # When
        lower_error_list, upper_error_list = compute_prediction_interval(
            RfStub(), data_eval, percentile=90
        )
